...
class AerobicNLME:
    ...
    def get_individual_theta(
        ...
        scale_eta  = jnp.exp(
            posterior_samples.get("scale_eta_log", jnp.log(jnp.ones(D_THETA) * 0.3))
        )
    ...
    def make_cold_start_contexts(n_obs: int) -> jax.Array:
        ...
        Uses the population resting baseline X0_DEFAULT for all observations.
        Replace with actual UKF posterior means as history accumulates.

        Returns
        -------
        state_contexts : shape (n_obs, STATE_DIM) — all rows = X0_DEFAULT
        """
        from app.engine.assimilation.ukf_filter import X0_DEFAULT
        return jnp.tile(X0_DEFAULT[None, :], (n_obs, 1))
